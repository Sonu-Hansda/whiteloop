...
import { Instagram, ArrowUpRight, Eye, Heart, MessageCircle, TrendingUp } from "lucide-react";
    ...
    <section id="case-studies" className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="relative z-10 section-container" ref={ref}>
        <div className="mb-16 text-center">
          <div className="pill-badge mb-4 mx-auto">Case Studies</div>
          <motion.h2
          ...
          >
            A closer look at the founder's campaign, metrics, and the exact content system that brought organic authority and new inbound leads.
          </motion.p>
        </div>
          ...
          >
            <div className="glass-card p-8 border-primary/20 relative overflow-hidden flex flex-col items-center text-center">
              ...
              <div className="relative w-20 h-20 rounded-full border-2 border-primary p-1 bg-neutral-900 mb-4 mt-2">
                <div className="w-full h-full rounded-full bg-linear-to-tr from-yellow-400 via-pink-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl font-display uppercase">
                  A
                </div>
              </div>
        ...
        <div>
          <div className="mb-10 text-center md:text-left">
            <h3 className="text-2xl font-bold font-display text-foreground">Top Performing Content</h3>
            <p className="text-sm text-muted-foreground mt-1">
              Click any post below to watch it directly on Instagram and see the metrics in action.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {instagramPosts.map((post, i) => (
              ...
              >
                <PostImage src={post.imagePath} alt={post.title} gradient={post.gradient} />
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-primary uppercase tracking-wider font-display">{post.title}</span>
                      <Instagram className="w-4 h-4 text-muted-foreground group-hover:text-primary transition-colors" />
                    </div>
                    <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed">{post.caption}</p>
                  </div>
